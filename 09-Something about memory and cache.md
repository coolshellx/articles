...
在本例中，第一种方式（触发缺页中断）与第二种方式（初始化时分配物理内存）的时间差为64ms，这个时间差基本就是操作系统处理缺页中断并完成物理内存分配所需的时间。我们分配了32*1024*1024字节的内存，而内存页的size一般为4096字节，所以一共触发了32*1024*1024/4096次缺页中断,对么？错误。虽然内存页的size一般为4096字节，但是操作系统以及硬件并不是以4096字节为单位映射内存，而是以一个略大的单位--一般是4096的整数倍(比如1MB)分配物理内存，具体细节因操作系统以及物理硬件而异。
